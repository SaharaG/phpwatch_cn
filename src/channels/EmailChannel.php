<?php
    ...
    class EmailChannel extends Channel
    {
        ...
        {   
            $values = array( 'loginfo' => '发送方式：EmailChannel，内容：' . $this->getMessage($monitor));
            if(defined('MAIL_TYPE') && MAIL_TYPE=='client'){
                ...
                $values = array( 'loginfo' => '发送方式：Mailer，状态：'. $result . '，内容：' . $this->getMessage($monitor));
            }else{
                mail($this->config['address'], $this->getSubject($monitor), $this->getMessage($monitor));
                $values = array( 'loginfo' => '发送方式：mail，内容：' . $this->getMessage($monitor));
            }
